...
- Last completed task: v07-14 job wiki first E2E
- Next task: v07-15 source summary fallback E2E
...
4. `docs/plans/v07/2026-05-17-v07-15-source-summary-fallback-e2e.md`
...
- v07-15: job wiki가 없을 때 source summary fallback을 E2E로 검증한다.
...
python -m py_compile src/retrieval/planner.py tools/ask.py
```

결과:

- `tests.test_v07_ask_cli`: 6 tests OK
- `py_compile`: OK
- `finish_task.py`: 271 tests OK, docs freshness OK, Notion handoff dry-run OK
...
v07을 계속한다면 v07-15부터 시작한다. 먼저 v07-15 plan을 읽고, red test를 작성한 뒤 구현한다. 작업 범위가 handoff 구조 자체라면 `docs/handoff/README.md`의 규칙을 우선 따른다.
